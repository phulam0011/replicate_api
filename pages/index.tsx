...
export default function Home() {
  ...
  async function startRecording() {
    await navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      ...
      media.onstop = async function () {
        ...
        makePredictionRequest(response);
  ...
  async function makePredictionRequest(response: any) {
    const apiUrl = 'https://api.replicate.com/v1/predictions';
    const inputText = response.text; // Thay thế bằng văn bản đầu vào thật sự

    const postData = {
      version: 'eb4d8a28a70f76101cc296153ba417ff014fc28ae4a48328e97f12e893038993',
    ...
    const response_2 = await axios.post(apiUrl, postData, { headers });
    const predictionId = response_2.data.urls.get;
    // const predictionUrl = `${apiUrl}/${predictionId}`;
    const getResult = await axios.get(predictionId, { headers });
    console.log(getResult.data.output);
    displayResult(getResult);
    // Xử lý dữ liệu trả về ở đây nếu cần thiết
  }


  function displayResult(getResult: any) {
    setQuery(getResult.data.output);
  }
